...
| **`monthly_installment` orphan column (pending p9 drop)** | `supabase/migrations/20260101000000_baseline_schema.sql:129`, `app/(system)/dashboard/cars/actions.ts` (`mapFormToRow`), `lib/supabase/mappers.ts` | Baseline column with **no writer** — P8 replaced it with `installment_monthly` and the form path was retired. `mapDbCarToCar` still *reads* it into `pricing.monthlyInstallment` for pre-existing rows. Pending a **p9 drop migration** (idempotent `DROP COLUMN IF EXISTS`, back up/backfill first). |
| **Parallel/duplicate filter systems** | live: `components/CarsFilters.tsx` + `components/pages/CarsListingPage.tsx`; unused abstraction: `hooks/useVehicleFilters.ts` + `lib/vehicles/filters/applyFilters.ts` + `VehicleFilterState` | Two filter implementations coexist; the `useVehicleFilters` / `lib/vehicles/filters` pair (incl. the `financing` facet) is **not wired to the live fleet page**, which filters inline. Consider consolidating onto one. |
| **Hero `isSearching` debounce mirrored** | `components/hero/HeroSearchPanel.tsx` (`useDebouncedValue`, 250 ms) vs `hooks/useHeroSearch.ts` | The dropdown loading flag re-runs the **same 250 ms debounce** in the panel instead of `useHeroSearch` exposing it. Minor cleanup: surface it from the hook to avoid the duplicated timer. |
